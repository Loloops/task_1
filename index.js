window.addEventListener('DOMContentLoaded', () => {
  ...
  const createTabs = () => {
    categories.forEach((title) => {
      ...
      tab.innerHTML = `
        <input type="radio" id="${title.categoryId}" name="tab-group" />
        <label for="${title.categoryId}" class="tab-title">${title.categoryName}</label>
      ...
      products.forEach((prod) => {
        ...
        item.classList.add('tabs-content__item');
        if (prod.categoryId === title.categoryId) {
          item.innerHTML = `
          <img width="75" height="75" src="./assets/img/tovar.jpg" alt="poductPhoto" />
  ...
  tabs.addEventListener('click', (e) => {
    ...
    if (target && target.classList.contains('tab-title')) {
      tab.forEach((item, i) => {
